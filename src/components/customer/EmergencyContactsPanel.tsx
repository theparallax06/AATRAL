    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">
              {emergencyContacts.length} of 3 contacts saved — used automatically during SOS alerts
            </p>
          </div>
        </div>
        {canAdd && (
      ...
      <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-amber-50 border border-amber-200">
        ...
        <p className="text-[11px] text-amber-800 leading-relaxed">
          During an active service booking, pressing the <strong>🚨 SOS</strong> button will instantly send your name, booking details, and live GPS location to all <strong>active</strong> contacts via SMS and WhatsApp — no additional input required.
        </p>
      </div>
      ...
      {emergencyContacts.length === 0 && !showForm && (
        <div className="p-10 text-center rounded-3xl border-2 border-dashed border-slate-200 bg-[#FAF9F6]">
          ...
          <p className="text-xs text-slate-400 mt-1 mb-4">Add up to 3 trusted people who will be alerted in case of emergency.</p>
          <button
            onClick={openAdd}
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                >
                  {contact.isActive ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <XCircle className="w-4 h-4" />}
                </button>
                <button
                  title="Edit"
      ...
      {!canAdd && !showForm && (
        <div className="text-center p-3 rounded-2xl bg-amber-50 border border-amber-200">
          <p className="text-xs text-amber-700 font-semibold">
            ✓ Maximum 3 contacts reached. Remove an existing contact to add a new one.
          </p>
        </div>
      )}
    </div>
