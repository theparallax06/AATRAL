...
// WhatsApp-green and SMS-blue for visual cues
...
const buildMessage = (alert: SosAlert): string => {
  const time = new Date(alert.timestamp).toLocaleString('en-IN', {
    dateStyle: 'medium',
    timeStyle: 'short',
  });
  return `🚨 EMERGENCY ALERT — AATRAL SAFETY

${alert.customerName} has triggered an SOS during a service booking.

📍 Location: ${alert.location.address}
🗺️ Live Track: maps.google.com/?q=${alert.location.lat.toFixed(6)},${alert.location.lng.toFixed(6)}

📋 Booking: ${alert.bookingNumber} (${alert.serviceCategory})
👷 Service Worker: ${alert.workerName} | ${alert.workerPhone}
📞 Customer Phone: ${alert.customerPhone}
🕐 Time: ${time}
...
export const SosModal: React.FC<SosModalProps> = ({ booking, onClose }) => {
  const {
    emergencyContacts,
    triggerSOS,
    sosAlerts,
  ...
  const locationAddress = userLocation?.formattedAddress
  ...
  // Poll for alert updates from context (status changes via setTimeout in AppContext)
  ...
  const handleActivateSOS = () => {
    setScreen('sending');
    setTimeout(() => {
      const alert = triggerSOS(booking.id);
      if (alert) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-black text-lg tracking-tight">
                  {screen === 'confirm' && '🚨 Emergency SOS'}
                  {screen === 'sending' && 'Activating SOS…'}
                  {screen === 'success' && 'SOS Alert Sent'}
                </h2>
                <p className="text-red-200 text-xs mt-0.5">
                  {screen === 'confirm' && 'Review and activate emergency alert'}
                  {screen === 'sending' && 'Please wait while we notify your contacts'}
                  {screen === 'success' && 'Your emergency contacts have been notified'}
                </p>
              </div>
            </div>
            {screen !== 'sending' && (
              ...
              {activeContacts.length === 0 ? (
                <div className="p-5 rounded-2xl bg-amber-50 border border-amber-200 text-center space-y-2">
                  ...
                  <p className="text-xs text-amber-700">
                    Please go to the <strong>Safety & Contacts</strong> tab and add at least one active emergency contact before using SOS.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="p-4 rounded-2xl bg-[#FAF9F6] border border-slate-200 space-y-2">
                    <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Active Booking</p>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-[#062B3A]">{booking.serviceCategoryName}</span>
                      <span className="font-mono text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded">{booking.bookingNumber}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-slate-600">
                      <User className="w-3.5 h-3.5 text-slate-400" />
                      Worker: <span className="font-semibold">{booking.assignedWorkerName}</span>
                      <span className="mx-1 text-slate-300">|</span>
                      <Phone className="w-3.5 h-3.5 text-slate-400" />
                      <span className="font-mono">{booking.assignedWorkerPhone ?? 'N/A'}</span>
                    </div>
                  </div>
          ...
          {screen === 'sending' && (
            <div className="py-10 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">Notifying {activeContacts.length} emergency contact{activeContacts.length !== 1 ? 's' : ''}</p>
              </div>
